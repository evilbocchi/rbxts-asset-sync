...
/**
 * File extensions that should be embedded as raw text in the asset map
 * instead of being uploaded to Roblox.
 */
const TEXT_FILE_EXTENSIONS = [".md", ".txt", ".json", ".xml", ".yaml", ".yml", ".csv", ".log", ".ini", ".cfg", ".conf"];
...
/**
 * Mapping of text file paths to their raw content.
 * Used to embed text files directly in the asset map.
 */
export const pathToTextContentMap: Record<string, string> = {};
...
export function setTextContent(filePath: string, content: string): void {
	if (pathToTextContentMap[filePath] === content) {
		return;
	}

	pathToTextContentMap[filePath] = content;
	assetMapDirty = true;
}
...
function removeTextContent(filePath: string): void {
	if (!(filePath in pathToTextContentMap)) {
		return;
	}

	delete pathToTextContentMap[filePath];
	assetMapDirty = true;
}
...
async function saveAssetMap() {
	...
	// Add uploaded assets (rbxassetid references)
	const assetKeys = Object.keys(pathToAssetIdMap).sort();
	for (const filePath of assetKeys) {
		const assetId = pathToAssetIdMap[filePath];
		const normalizedPath = filePath.replace(/\\/g, "/");
		lines.push(`  "${normalizedPath}": "rbxassetid://${assetId}",`);
	}

	// Add text files (raw content)
	const textKeys = Object.keys(pathToTextContentMap).sort();
	for (const filePath of textKeys) {
		const content = pathToTextContentMap[filePath];
		const normalizedPath = filePath.replace(/\\/g, "/");
		// Escape the content for TypeScript string literal
		const escapedContent = JSON.stringify(content);
		lines.push(`  "${normalizedPath}": ${escapedContent},`);
	}

	lines.push("} as const;\n");
	lines.push("export function getAsset(path: keyof typeof assets): string {\n  return assets[path];\n}");

	const totalEntries = assetKeys.length + textKeys.length;
	await fs.promises.writeFile(assetMapOutputPath, lines.join("\n"));
	assetMapDirty = false;
	LOGGER.info(
		`Asset map generated with ${totalEntries} entries (${assetKeys.length} uploaded, ${textKeys.length} text) at ${assetMapOutputPath}`,
	);
}
...
export async function syncAssetFile(filePath: string): Promise<string | undefined> {
	...
	// Check if this is a text file that should be embedded
	if (TEXT_FILE_EXTENSIONS.includes(ext)) {
		const textContent = fs.readFileSync(filePath, "utf8");
		setTextContent(filePath, textContent);
		LOGGER.info(`Embedded text content from ${filePath}`);
		return undefined;
	}
